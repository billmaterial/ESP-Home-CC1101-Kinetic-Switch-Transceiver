...
//############################################################################## SL to show Data Rate
  uint8_t CFG4 = this->read_status_register_(CC1101_MDMCFG4);
  uint8_t CFG3 = this->read_status_register_(CC1101_MDMCFG3);
  uint8_t dRate = ((256 + 47) * (pow(2, 10)) / pow(2, 28)) * 26000000 / 1000;
  ESP_LOGCONFIG(TAG, "  CC1101 DataRate: %d Kbps", dRate);
//##############################################################################
...
void CC1101::set_mode_(bool s) {
  ...
  if (s) {
    ...
    this->write_register_(CC1101_MDMCFG3, 0x47); //SL Set DataRate 30KBPS = 0x47, Old 0x93
    this->write_register_(CC1101_MDMCFG4, 10 + this->m4rxbw_); //SL Set DataRate 30KBPS = 10, Old 7
  }
